-- Lock down users and agencies tables with proper helper functions
BEGIN;
...
-- UPDATE/INSERT/DELETE: superadmin only (or service role via edge functions)
CREATE POLICY "users_modify_superadmin" ON public.users
...
-- Modify: superadmin only
CREATE POLICY "agencies_modify_superadmin" ON public.agencies
FOR INSERT, UPDATE, DELETE TO authenticated
USING (public.rls_auth_role() = 'superadmin')
WITH CHECK (public.rls_auth_role() = 'superadmin');
